...
from genweb6.core.utils import pref_lang, create_simple_vocabulary
...
class Destacats(Tile):
        ...
        if not self.tags and not self.portal_types:
            ...
            if self.portal_types in [u'File']:
                ...
                results = catalog(portal_type=self.portal_types,
                                  ...
                                  sort_order='reverse',
                                  sort_limit=1)
        item = self.filterObjects(results) if len(results) > 0 else None
        return item[0]
    ...
    def getNDestacats(self, limit):
        ...
        tags = self.tags[:]
        tags.append('#gran')
        #ubjects = [t.encode('utf-8') if isinstance(t, unicode) else t for t in tags]
        subjects = [t for t in self.tags]
